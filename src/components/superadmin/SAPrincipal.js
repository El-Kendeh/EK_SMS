...
export default function SAPrincipal() {
  ...
  async function handleSave(form) {
    try {
      ...
      if (editItem) {
        await req('PUT', `/api/principals/${editItem.id}/`, fd, true);
        showToast('Principal updated', 'success');
      } else {
        await req('POST', '/api/principals/', fd, true);
  ...
  async function handleToggle(item) {
    try { await req('PATCH', `/api/principals/${item.id}/toggle/`); load(); showToast('Status toggled', 'success'); } catch (e) { showToast(e.message, 'error'); }
  }

  async function handleBlock(item) {
    try { await req('PATCH', `/api/principals/${item.id}/block/`); load(); showToast(item.status === 'blocked' ? 'Unblocked' : 'Blocked', 'success'); } catch (e) { showToast(e.message, 'error'); }
  }

  async function handleDelete(item) {
    try { await req('DELETE', `/api/principals/${item.id}/`); load(); setDeleting(null); showToast('Deleted', 'success'); } catch (e) { showToast(e.message, 'error'); }
  }
  ...
  const fields = [
    ...
    { key: 'password', label: 'Password', type: 'text', placeholder: 'default: Principal@123' },
  ...
  return (
    <div className="sa-page">
      ...
      <div className="sa-page-head">
        <div>
          <h1 className="sa-page-title">Principals</h1>
          <p className="sa-page-sub">Full school principal registration with login credentials</p>
        </div>
        <button className="sa-btn sa-btn--primary" onClick={() => { setEditItem(null); setShowModal(true); }}><IcPlus /> Add Principal</button>
      </div>

      <div className="sa-toolbar">
        <div className="sa-search-bar">
          <input className="sa-search-input" placeholder="Search principals..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
        </div>
      </div>
        ...
        <>
          <div className="sa-table-wrap">
            <table className="sa-table">
              <thead>
                <tr>
                  <th>Name</th><th>Employee ID</th><th>Email</th><th>Phone</th><th>Status</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
                {paged.length === 0 ? (
                  <tr><td colSpan={6} style={{ textAlign: 'center', padding: 40, color: 'var(--sa-text-3)' }}>No principals found</td></tr>
                ) : paged.map(p => (
                  <tr key={p.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 4 }}>
                        ...
                        <button className="sa-btn sa-btn--ghost sa-btn--sm" title="Toggle active" onClick={() => handleToggle(p)}><span style={{ fontSize: 12 }}>{p.is_active ? '🔴' : '🟢'}</span></button>
      ...
      {deleting && (
        <div className="sa-modal-overlay" onClick={() => setDeleting(null)}>
          <div className="sa-modal" onClick={e => e.stopPropagation()}>
            <h3>Delete this principal?</h3>
            <p style={{ color: 'var(--sa-text-3)', margin: '8px 0 20px' }}>This will also remove their login access. This cannot be undone.</p>
...
function PrincipalForm({ fields, editItem, onSave, onClose }) {
  ...
  useEffect(() => {
    if (editItem) {
      const f = {};
      fields.forEach(({ key }) => { f[key] = editItem[key] || ''; });
      setForm(f);
    }
  }, [editItem, fields]);
  ...
  return (
    <div className="sa-modal-overlay" onClick={onClose}>
      <div className="sa-modal sa-modal--lg" onClick={e => e.stopPropagation()}>
        <h3>{editItem ? 'Edit Principal' : 'Register Principal'}</h3>
