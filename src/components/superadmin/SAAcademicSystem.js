...
export default function SAAcademicSystem() {
  ...
  async function handleSave(form) {
    try {
      if (editItem) { await req('PUT', `/api/academic-systems/${editItem.id}/`, form); showToast('Updated', 'success'); }
      else { await req('POST', '/api/academic-systems/', form); showToast('Created', 'success'); }
      setShowModal(false); setEditItem(null); load();
    } catch (e) { showToast(e.message, 'error'); }
  }
  async function handleToggle(item) { try { await req('PATCH', `/api/academic-systems/${item.id}/toggle/`); load(); } catch (e) { showToast(e.message, 'error'); } }
  async function handleDelete(item) { try { await req('DELETE', `/api/academic-systems/${item.id}/`); load(); setDeleting(null); } catch (e) { showToast(e.message, 'error'); } }
  ...
  return (
    <div className="sa-page">
      ...
      <div className="sa-page-head">
        <div>
          <h1 className="sa-page-title">Academic Systems</h1>
          <p className="sa-page-sub">Curriculum frameworks / academic systems used across schools</p>
        </div>
        <button className="sa-btn sa-btn--primary" onClick={() => { setEditItem(null); setShowModal(true); }}><IcPlus /> Add Academic System</button>
      </div>
      {loading ? <div className="sa-loading">Loading...</div> : (
        <div className="sa-table-wrap">
          <table className="sa-table">
            <thead><tr><th>ID</th><th>Academic System Name</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
              {list.length === 0 ? (
                <tr><td colSpan={4} style={{ textAlign: 'center', padding: 40, color: 'var(--sa-text-3)' }}>No academic systems found</td></tr>
              ) : list.map(p => (
                <tr key={p.id}>
      ...
      {showModal && (
        <div className="sa-modal-overlay" onClick={() => { setShowModal(false); setEditItem(null); }}>
          <div className="sa-modal" onClick={e => e.stopPropagation()}>
            <h3>{editItem ? 'Edit Academic System' : 'Add Academic System'}</h3>
            <AcademicSystemForm editItem={editItem} onSave={handleSave} onClose={() => { setShowModal(false); setEditItem(null); }} />
          </div>
        </div>
      )}
      {deleting && (
        <div className="sa-modal-overlay" onClick={() => setDeleting(null)}>
          <div className="sa-modal" onClick={e => e.stopPropagation()}>
            <h3>Delete this academic system?</h3>
...
function AcademicSystemForm({ editItem, onSave, onClose }) {
    ...
    <form onSubmit={e => { e.preventDefault(); if (!name.trim()) return; onSave({ name: name.trim() }); }}>
      <div className="sa-modal-body">
        <div className="sa-field">
          <label>Academic System Name *</label>
          <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. British, American, Nigerian" autoFocus />
        </div>
      </div>
