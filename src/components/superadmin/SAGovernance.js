...
const ROLE_ICONS = { superadmin: <IcShield />, principal: <IcSchool />, teacher: <IcPerson /> };
...
/* ================================================================
   ...
   ================================================================ */
function CreateRoleModal({ existing, initial, onClose, onCreated }) {
  const [name, setName]               = useState(initial?.name ? `${initial.name} (copy)` : '');
    ...
    <div style={{ position: 'relative' }}>
      ...
      {showCreate && (
        <CreateRoleModal
          existing={ROLES}
          initial={cloneSeed}
          onClose={() => { setShowCreate(false); setCloneSeed(null); }}
          onCreated={(role) => {
        ...
        <div key={role.id} className="sa-role-card">
          ...
          <div className="sa-role-footer">
            ...
            <div className="sa-role-btn-row">
              <button className="sa-role-icon-btn" aria-label="Clone role" onClick={() => { setCloneSeed({ name: role.name, desc: role.desc }); setShowCreate(true); }}><IcCopy size={16} /></button>
              <button
