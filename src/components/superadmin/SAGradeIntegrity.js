...
export default function SAGradeIntegrity({ onDetail }) {
  ...
  const counts = useMemo(() => {
    const c = { all: MOCK_REQUESTS.length, Pending: 0, Approved: 0, Flagged: 0, Rejected: 0 };
    MOCK_REQUESTS.forEach(r => { if (c[r.status] !== undefined) c[r.status]++; });
    return c;
  }, []);
  ...
  const schools = useMemo(() => [...new Set(MOCK_REQUESTS.map(r => r.school))], []);
