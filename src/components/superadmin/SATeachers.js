...
export default function SATeachers() {
  ...
  async function handleSave(form) {
    try {
      ...
      if (editItem) { await req('PUT', `/api/teachers/${editItem.id}/`, fd, true); showToast('Updated', 'success'); }
      else { await req('POST', '/api/teachers/', fd, true); showToast('Created', 'success'); }
      setShowModal(false); setEditItem(null); load();
    } catch (e) { showToast(e.message, 'error'); }
  }

  async function handleToggle(item) { try { await req('PATCH', `/api/teachers/${item.id}/toggle/`); load(); } catch (e) { showToast(e.message, 'error'); } }
  async function handleBlock(item) { try { await req('PATCH', `/api/teachers/${item.id}/block/`); load(); } catch (e) { showToast(e.message, 'error'); } }
  async function handleDelete(item) { try { await req('DELETE', `/api/teachers/${item.id}/`); load(); setDeleting(null); } catch (e) { showToast(e.message, 'error'); } }
  ...
  const fields = [
    ...
    { key: 'password', label: 'Password', type: 'text', placeholder: 'default: Teacher@123' },
    ...
    { key: 'subjects_specialization', label: 'Subjects', type: 'text' },
    { key: 'hire_date', label: 'Hire Date', type: 'date' },
    { key: 'contract_type', label: 'Contract', type: 'select', options: ['Full-time', 'Part-time', 'Contract'] },
    ...
    <div className="sa-page">
      ...
      <div className="sa-page-head">
        <div>
          ...
          <p className="sa-page-sub">Full teacher registration with login credentials</p>
        </div>
        <button className="sa-btn sa-btn--primary" onClick={() => { setEditItem(null); setShowModal(true); }}><IcPlus /> Add Teacher</button>
      </div>
      <div className="sa-toolbar">
        <div className="sa-search-bar">
          <input className="sa-search-input" placeholder="Search teachers..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
        </div>
      </div>
        ...
        <>
          <div className="sa-table-wrap">
            <table className="sa-table">
              ...
              <tbody>
                {list.length === 0 ? (
                  <tr><td colSpan={6} style={{ textAlign: 'center', padding: 40, color: 'var(--sa-text-3)' }}>No teachers found</td></tr>
                ) : list.map(p => (
                  <tr key={p.id}>
...
function StaffForm({ fields, editItem, onSave, onClose }) {
  ...
  useEffect(() => { if (editItem) { const f = {}; fields.forEach(({ key }) => { f[key] = editItem[key] || ''; }); setForm(f); } }, [editItem, fields]);
  function set(k, v) { setForm(p => ({ ...p, [k]: v })); }
  return (
    <div className="sa-modal-overlay" onClick={onClose}>
      <div className="sa-modal sa-modal--lg" onClick={e => e.stopPropagation()}>
        <h3>{editItem ? 'Edit Teacher' : 'Register Teacher'}</h3>
