...
function StudentsPage({ school }) {
  ...
  const openEdit = s => {
    setForm({ first_name: s.first_name, last_name: s.last_name, email: s.email,
      ...
      date_of_birth: s.date_of_birth || '', phone_number: s.phone_number || '' });
    setError(''); setModal(s);
  };
...
export default function SchoolAdminDashboard({ onNavigate }) {
  ...
  else if (activePage === 'students')   pageContent = <StudentsPage school={school} />;
  ...
  return (
    <div className="ska-wrap">
      ...
      {/* FAB */}
      <button className="ska-fab" aria-label="Add new">
        <Ic name="add" />
      </button>
    </div>
  );
