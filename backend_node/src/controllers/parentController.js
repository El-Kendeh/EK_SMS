...
async function loadReportCardGrades(childId, termId) {
...
// Canonical JSON of the published grade rows — the tamper-evidence fingerprint.
// Shared with the public verify route so both sides hash identically.
...
async function getChildReportCards(req, res) {
  try {
    ...
    const grades = await loadReportCardGrades(childId, term_id);
    ...
    // Academic year labels for the terms involved.
    ...
    // Verification receipts: attach the hash only when the receipt still
    // matches the CURRENT published content (a stale hash must not render as
    // "verified" next to grades it no longer fingerprints).
    const receipts = await ReportCardReceipt.findAll({
      where: { student_id: childId },
      raw: true,
    });
    const receiptByTerm = {};
    ...
    const reportCards = Object.values(terms).map(t => {
      ...
      const currentHash = reportCardContentHash(childId, t.termId, gradesByTerm[Number(t.termId)] || []);
...
async function downloadChildReportCard(req, res) {
  try {
    ...
    // Tamper-evidence: hash the published set, upsert the public receipt.
    const contentHash = reportCardContentHash(childId, termId, grades);
    ...
    const studentName = fullName(student?.user) || `Student #${childId}`;
